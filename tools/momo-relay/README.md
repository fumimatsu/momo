...
充填の基準時間は順位そのものではなくRace Controlの`intervalToAheadMs`と`lapDeltaToAhead`で決める。
前者は後続車の最新共通マーカーにおける秒差で、完了周回数の境界をまたいでも有効である。後者は先行車が
同じマーカーを次周以降ですでに通過した確定周回差であり、単純な完了周回数の差ではない。先頭は45秒、
秒差では0秒差の40秒から8秒差以上の20秒まで線形に短縮し、確定1周遅れは16秒、3周差以上は12秒とする。
...
M5StickS3 telemetryが現在のupstream接続世代で`fuel_command_v1` capabilityを通知した場合だけ、
車両へ転送するRC commandへRelay正本のFuel残量を`F:0..100`として追加する。通常形は
