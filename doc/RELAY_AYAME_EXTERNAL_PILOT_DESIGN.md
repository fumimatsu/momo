...
`planned`。この文書は設計だけを定める。Relay、Viewer、Pi/Momo、Ayame VPS の実装変更は
この段階では行わない。

現在の優先順位は、ローカル 4 台の映像計測、Observer / Unity 連携、Relay の運用安定化、
および Pilot 側 FFB の実機調整である。外部 Pilot はこれらの受け入れ条件を満たした後に扱う。
...
- Relay が切断、lease 失効、Pilot DataChannel 切断、または deadman timeout を検知したら、Pi へ
  neutral / Drive OFF を送る既存 failsafe を必ず通す。
...
### Phase 1: Relay の Ayame 下流接続

- Relay に source ごとの Ayame client を追加する。
- 1 source、1 Pilot、会場 LAN 内の試験端末で映像のみを検証する。
- RTP 転送、keyframe 要求、切断／再接続を Local Relay Pilot と同じ基準で確認する。

### Phase 2: `relay-ayame` Viewer

- Ayame signaling と `momo-command` / `momo-telemetry` / `momo-race` を組み合わせる Viewer mode を追加する。
- 外部 Pilot PC の FFB Bridge、Race HUD、telemetry 表示を確認する。
